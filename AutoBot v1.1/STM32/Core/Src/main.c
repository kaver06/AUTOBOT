...
volatile uint8_t tx_busy = 0;      // 1 while UART IT transmit ongoing
/* USER CODE END PV */
  ...
  while (1)
  {
	...
	/*
	  ...
	  if (send_flag)
	  {
	      ...
	      if (tx_busy)
	      {
	          // busy: next timer tick will attempt again
	      }
	      else
	      {
	          // Format safely into a local stack buffer first
	          char local_buf[512];
